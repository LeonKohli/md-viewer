                ...
                >
                  <div class="flex items-center gap-2">
                    <div class="flex-1 relative">
                      <Input
                        v-model="file.name"
                        :placeholder="index === 0 ? 'document' : 'filename'"
                        required
                        class="pr-16"
                      />
                      <span 
...
watch([() => props.modelValue, () => props.existingGist], ([isOpen, gist]) => {
  ...
  if (gist) {
    ...
    const currentFile = props.currentFilename || 'document.md'
    const currentFileBase = currentFile.endsWith('.md') ? currentFile.slice(0, -3) : currentFile
    files.value.push({
...
const handleSubmit = async () => {
  ...
  try {
    ...
    files.value.forEach(file => {
      ...
      // GitHub API doesn't allow empty files - use a period as placeholder
